...
//Dobijamo dva parametra od znacaja:ioctl_num-broj ioctl-a koji nas proziva i parametra koji je prosledjen ioctl funkciji
		 ...
		 unsigned long ioctl_param)
{
	...
	case funkcije: //primamo razne naredbe iz user space
      ...
      // primeri raznih funkcija iz softverski_init
 	...
 	if(naredbe_data[1]==3)      lcd_putChar( 200,200,'f');//ispise slovo f na lokaciji 200,200
  	if(naredbe_data[1]==4)      lcd_point(10,10,RED);//nacrta crvenu tacku na koordinatama 10,10 
        ...
        break;
	case slika:
               temp_slika = (unsigned short *)ioctl_param;
